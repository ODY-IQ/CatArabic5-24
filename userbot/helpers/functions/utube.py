...
async def ytsearch(query, limit):
    ...
    for v in videolinks.result()["result"]:
        ...
        try:
            textresult += f"**▾∮ الوصف : **`{v['descriptionSnippet'][-1]['text']}`\n"
        except Exception:
            textresult += "**▾∮ الوصف : **`لا يوجد`\n"
        textresult += f"**▾∮ المدة : **__{v['duration']}__  **المشاهدات : **__{v['viewCount']['short']}__\n"
        result += f"☞ {textresult}\n"
    return result
...
async def result_formatter(results: list):
    ...
    for index, r in enumerate(results, start=1):
        ...
        out += f'<b>❯  المدة 🕐:</b> <i>{r.get("accessibility").get("duration")}</i>\n'
        views = f'<b>❯  المشاهدات 📉:</b> <i>{r.get("viewCount").get("short")}</i>\n'
        out += views
        out += f'<b>❯  تاريخ الرفع 📅:</b> <i>{r.get("publishedTime")}</i>\n'
        if upld:
            out += "<b>❯  تم رفعها من قبل 👨‍💻:</b> "
...
def download_button(vid: str, body: bool = False):  # sourcery no-metrics
    ...
    buttons = [
        [
            Button.inline("⭐️ الافضل - 📹 صيغة MKV", data=f"ytdl_download_{vid}_mkv_v"),
            Button.inline(
                "⭐️ الافضل - 📹 WebM/MP4",
    ...
    buttons += [
        [Button.inline("⭐️ الافضل - 🎵 320Kbps - MP3", data=f"ytdl_download_{vid}_mp3_a")]
    ]
    buttons += sublists(
        [
...
def _tubeDl(url: str, starttime, uid: str):
    ...
    try:
        ...
        LOGS.error(
            "خطأ: القائم بالتحميل لم يجعل هذا الفيديو متاحًا في بلدك"
        )
    else:
        return x
