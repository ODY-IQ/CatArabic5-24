    ...
    def filter(self, event):
        ...
        if self.require_admin and not isinstance(event._chat_peer, types.PeerUser):
            ...
            if not is_creator and not is_admin:
                text = "**▾↫ عذرًاأحتاج إلى حقوق المشرف لأتمكن من استخدام هذا الأمر! ✘**"
...
async def send_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msg = f"**عذرًا ، لا يمكنني إرسال هذه الرسالة في الدردشات العامة ، فقد تحتوي على بعض البيانات الحساسة ، لذا تحقق من** [مجموعة سجل البوت]({msglink})."
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            reply_to=reply_to,
