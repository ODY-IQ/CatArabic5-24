...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Catuserbot"):
            buttons = [
                (
                    ...
                    Button.url("Repo", "https://t.me/NNEEE"),
                )
            ]
            ...
            result = builder.article(
                title="November Help",
                description="**▾∮ قائمة التعليمات الخاصة بــ نوفمبر ⁂**",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/nneee"),
                Button.url(
                    "المساعدة",
                    "t.me/oorrr_bot",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل نوفمبر من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="▾∮ՏøuƦcε πøνεʍβεƦ 🌦",
